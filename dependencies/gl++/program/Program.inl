  ...
  ::getInfoLog(std::string &log) const
{
  ...
  if(len != 0)
  {
    ...
    // get log
    int dummy = 0;
    glGetProgramInfoLog(getIdentifier(), len, &dummy, &log[0]);
  } // end if
...
void Program
  ::setUniform2i(const GLchar *name,
                 const GLint v0,
                 const GLint v1) const
{
  VariableMap::const_iterator u = mUniformMap.find(name);
  if(u != mUniformMap.end())
    glUniform2i(u->second,v0,v1);
} // end Program::setUniform4fv()

void Program
  ::setUniform2f(const GLchar *name,
                 const GLfloat v0,
                 const GLfloat v1) const
{
  VariableMap::const_iterator u = mUniformMap.find(name);
  if(u != mUniformMap.end())
    glUniform2f(u->second,v0,v1);
} // end Program::setUniform4fv()
...
#else
  std::cerr << "Program::setParameteri(): Ubuntu's version of glew doesn't support glProgramParameteriEXT()." << std::endl;
#endif
